...
/*   		$("td:visible  input[name^=msgFldStartSituVal]").each(function(){
		...
		if ($('select[name=extAdapter]').val()=="" && $('select[name=svcMotivUseDstcd]').val()=="ASYN_SYNC" ){
 		...
 		if ($("select[name=fromAdapter] option:selected").attr("std") == "K"){
			...
			if($("input[name^=serviceId_"+gbn+"_Source]:visible").val().trim() ==""){				
				if( gbn == "RES" ){	
					var ret = confirm("서비스ID가 입력되지 않았습니다. \n공백으로 처리하면 요청의 서비스ID를 참조합니다. \n계속하시겠습니까?");
					if(ret){
						$("input[name^=serviceId_"+gbn+"_Source]:visible").val(" ");
					}else{
	...
	function changeDirection(motive,base){
		...
		//if(gbn == "" || gbn == null) return;
			...
			var $a = $("div[name=block_REQ]").find($("div[name=left_td]"));
			...
			$a = $("div[name=block_REQ]").find($("div[name=right_td]"));
			$a.find($("input[name^=layoutMapping]")).attr('name',"layoutMappingName_REQ_"+sright);
			$a.find($("img[name^=searchLayout]")).attr('name',"searchLayout_REQ_"+sright);
			$a.find($("img[name^=delLayout]")).attr('name',"delLayout_REQ_"+sright);
						
			$a = $("div[name=block_RES]").find($("div[name=left_td]"));
			...
			$a = $("div[name=block_RES]").find($("div[name=right_td]"));
			$a.find($("input[name^=layoutMapping]")).attr('name',"layoutMappingName_RES_"+rright);
			$a.find($("img[name^=searchLayout]")).attr('name',"searchLayout_RES_"+rright);
			$a.find($("img[name^=delLayout]")).attr('name',"delLayout_RES_"+rright);
			
			$a = $("div[name=block_ERR]").find($("div[name=left_td]"));
			$a.find($("input[name^=layoutMapping]")).attr('name',"layoutMappingName_ERR_"+rleft);
			$a.find($("img[name^=searchLayout]")).attr('name',"searchLayout_ERR_"+rleft);
			$a.find($("img[name^=delLayout]")).attr('name',"delLayout_ERR_"+rleft);

			$a = $("div[name=block_ERR]").find($("div[name=right_td]"));
			$a.find($("input[name^=layoutMapping]")).attr('name',"layoutMappingName_ERR_"+rright);
			...
			//img change
			//var direction = "";
			var sdirection = base=="D"?"right":"left";
			var rdirection = sdirection=="right"?"left":"right";
			$("div[name=block_REQ]").find($("img[name=bgimg]")).attr("src","<c:url value='/images/chart/"+sdirection+"_arrow.png' />");
			$("div[name=block_ERR],div[name=block_RES]").find($("img[name=bgimg]")).attr("src","<c:url value='/images/chart/"+rdirection+"_arrow.png' />");
			
			if(sdirection == "left"){
				$("div[name=block_REQ] img[name=adptImg1]").attr("src","<c:url value='/images/chart/psvAdapter1.png' />");
				$("div[name=block_REQ] img[name=adptImg2]").attr("src","<c:url value='/images/chart/gstatAdapter1.png' />");
				$("div[name=block_ERR] img[name=adptImg1],div[name=block_RES] img[name=adptImg1]").attr("src","<c:url value='/images/chart/gstatAdapter2.png' />");
				$("div[name=block_ERR] img[name=adptImg2],div[name=block_RES] img[name=adptImg2]").attr("src","<c:url value='/images/chart/psvAdapter2.png' />");
			}else{
				$("div[name=block_REQ] img[name=adptImg1]").attr("src","<c:url value='/images/chart/gstatAdapter2.png' />");
				$("div[name=block_REQ] img[name=adptImg2]").attr("src","<c:url value='/images/chart/psvAdapter2.png' />");			
				$("div[name=block_ERR] img[name=adptImg1],div[name=block_RES] img[name=adptImg1]").attr("src","<c:url value='/images/chart/psvAdapter1.png' />");
				$("div[name=block_ERR] img[name=adptImg2],div[name=block_RES] img[name=adptImg2]").attr("src","<c:url value='/images/chart/gstatAdapter1.png' />");
			}
	...
	function ctrlSvcIdBox(){
		...
		$("div[name=block_REQ]").find("TR").first().find("TD:nth-child(2)").css('visibility','hidden');			//bzwk
		$("div[name=block_REQ]").find("TR").first().find("TD:nth-child(4)").css('visibility','hidden');
		$("div[name=block_REQ]").find("TR").last().find("TD:nth-child(1)").css('visibility','hidden');			//serviceID
		$("div[name=block_REQ]").find("TR").last().find("TD:nth-child(3)").css('visibility','hidden');
		
		$("div[name=block_RES]").find("TR").first().find("TD:nth-child(4)").css('visibility','hidden');
		$("div[name=block_RES]").find("TR").first().find("TD:nth-child(2)").css('visibility','hidden');
		$("div[name=block_RES]").find("TR").last().find("TD:nth-child(3)").css('visibility','hidden');	
		$("div[name=block_RES]").find("TR").last().find("TD:nth-child(1)").css('visibility','hidden');		
    	...
    	var reqFld = $("div[name=reqFld]")[0].outerHTML;
    	var reqSvcId = $("div[name=reqSvcId]")[0].outerHTML;
    	var resFld = $("div[name=resFld]")[0].outerHTML;
    	var resSvcId = $("div[name=resSvcId]")[0].outerHTML;
    	var reqRefID = $("div[name=reqRefIDName]")[0].outerHTML;
    	var resRefID = $("div[name=resRefIDName]")[0].outerHTML;
    	var errRefID = $("div[name=errRefIDName]")[0].outerHTML;
    	var emptySpace = "<TD colspan=2 width='350px'></td>";
    	if(base =="D"){ //당발

    		if($("select[name=fromAdapter] option:selected").attr('std') != "K"){
				if(gbn == "REQ"){				
					$("div[name=block_REQ]").find("TR").last().find("TD:nth-child(1)").html(reqSvcId);
					$("div[name=block_REQ]").find("TR").last().find("TD:nth-child(3)").html('');
					$("div[name=block_REQ]").find("TR").last().find("TD:nth-child(1)").css('visibility','visible');
				}else{		//당발 응답
					$("div[name=block_RES]").find("TR").first().find("TD:nth-child(4)").html(resSvcId);
					$("div[name=block_RES]").find("TR").first().find("TD:nth-child(2)").html('');
					$("div[name=block_RES]").find("TR").first().find("TD:nth-child(4)").css('visibility','visible');

				}	
			}
			
	    		$("div[name=block_REQ]").find("TR").first().find("TD:nth-child(2)").html(reqFld);
	    		$("div[name=block_REQ]").find("TR").first().find("TD:nth-child(4)").html('');
	    		$("div[name=block_REQ]").find("TR").first().find("TD:nth-child(2)").css('visibility','visible');
	    	if(motive =="ASYN" || motive == "SYNC_ASYN"){	
	    		$("div[name=block_RES]").find("TR").last().find("TD:nth-child(3)").html(resFld);
	    		$("div[name=block_RES]").find("TR").last().find("TD:nth-child(1)").html('');
	    		$("div[name=block_RES]").find("TR").last().find("TD:nth-child(3)").css('visibility','visible');	
			}
			$("div[name=block_REQ]").find("TR:nth-child(2)").find("TD:nth-child(3)").html('');
			$("div[name=block_REQ]").find("TR:nth-child(2)").find("TD:nth-child(1)").html(reqRefID);
			
			$("div[name=block_RES]").find("TR:nth-child(3)").find("TD:nth-child(1)").html('');
			$("div[name=block_RES]").find("TR:nth-child(3)").find("TD:nth-child(3)").html(resRefID);
			
			$("div[name=block_ERR]").find("TR:nth-child(3)").find("TD:nth-child(1)").html('');
			$("div[name=block_ERR]").find("TR:nth-child(3)").find("TD:nth-child(3)").html(errRefID);
    		
    	}else if(base =="T"){
    		if($("select[name=fromAdapter] option:selected").attr('std') != "K"){
				if(gbn =="REQ"){
					$("div[name=block_REQ]").find("TR").last().find("TD:nth-child(3)").html(reqSvcId);
					$("div[name=block_REQ]").find("TR").last().find("TD:nth-child(1)").html('');
					
					$("div[name=block_REQ]").find("TR").last().find("TD:nth-child(3)").css('visibility','visible');
				}else{
					$("div[name=block_RES]").find("TR").first().find("TD:nth-child(2)").html(resSvcId);
					$("div[name=block_RES]").find("TR").first().find("TD:nth-child(4)").html('');
					
					$("div[name=block_RES]").find("TR").first().find("TD:nth-child(2)").css('visibility','visible');
				}
			}
			
	    		$("div[name=block_REQ]").find("TR").first().find("TD:nth-child(4)").html(reqFld);
	    		$("div[name=block_REQ]").find("TR").first().find("TD:nth-child(2)").html('');
	    		
	    		$("div[name=block_REQ]").find("TR").first().find("TD:nth-child(4)").css('visibility','visible');
	    	if(motive =="ASYN" || motive == "SYNC_ASYN"){	
	    		$("div[name=block_RES]").find("TR").last().find("TD:nth-child(1)").html(resFld);
	    		$("div[name=block_RES]").find("TR").last().find("TD:nth-child(3)").html('');
	    		
	    		$("div[name=block_RES]").find("TR").last().find("TD:nth-child(1)").css('visibility','visible');	
			}			
			$("div[name=block_REQ]").find("TR:nth-child(2)").find("TD:nth-child(1)").html('');
			$("div[name=block_REQ]").find("TR:nth-child(2)").find("TD:nth-child(3)").html(reqRefID);
			
			$("div[name=block_RES]").find("TR:nth-child(3)").find("TD:nth-child(3)").html('');
			$("div[name=block_RES]").find("TR:nth-child(3)").find("TD:nth-child(1)").html(resRefID);	
			
			$("div[name=block_ERR]").find("TR:nth-child(3)").find("TD:nth-child(3)").html('');
			$("div[name=block_ERR]").find("TR:nth-child(3)").find("TD:nth-child(1)").html(errRefID);	
			//$("div[name=block_REQ]").find("TR").first().find("TD:nth-child(4)").css('visibility','visible');
    	...
    	//이벤트 재 바인딩
		 ...
		 $("img[name^=searchbzwkFldName]").click(function(){	
		    ...
		    url = url + "?cmd=FIELD";
		    var ret = showModal(url,args,1000,600);
...
/* 		if(gbn == null || typeof gbn == undefined || gbn ==""){
			 return;
		} */
...
/* 			//표준 비표준에 따른..k
	...
	function modify(){
		...
		var serviceId = $("input[name=serviceId_"+gbn+"_Source]").val();
		var bzwkFldName1 = $("input[name=bzwkFldName1_"+gbn+"_Source]").val();
		var bzwkFldName2 = $("input[name=bzwkFldName2_"+gbn+"_Source]").val();
		var msgFldStartSituVal1= $("input[name=msgFldStartSituVal1_"+gbn+"_Source]").val();
		var msgFldStartSituVal2 = $("input[name=msgFldStartSituVal2_"+gbn+"_Source]").val();
		var msgFldLen1 = $("input[name=msgFldLen1_"+gbn+"_Source]").val();
		var msgFldLen2 = $("input[name=msgFldLen2_"+gbn+"_Source]").val();
...
/* 					if (typeof callback === 'function') {
	...
	function detail(key){
		isDetail = true;
		$("#btn_modify").attr('src',"<c:url value='/images/bt/bt_modify.gif'/>");
		$.ajax({
			type : "POST",
			url:'<c:url value="/onl/transaction/tracking/dbTrackingChartMan.json" />',
			dataType:"json",
			...
			success:function(json){
					$("input, select").val('');
						...
						$("input[name=bzwkFldName1_"+gbn+"_Source]").val(data["BZWKFLDNAME1"]);
						if(data["BZWKFLDNAME2"] != null && data["BZWKFLDNAME2"] !=""){
							$("input[name=bzwkFldName2_"+gbn+"_Source]").val(data["BZWKFLDNAME2"]);
							$("img[name=fldPlus_"+gbn+"_Source]").click();
						}else{
							$("img[name^=fldMin]").click();
						}
							
						$("input[name=msgFldStartSituVal1_"+gbn+"_Source]").val(data["MSGFLDSTARTSITUVAL1"]);
						$("input[name=msgFldStartSituVal2_"+gbn+"_Source]").val(data["MSGFLDSTARTSITUVAL2"]);
			
						$("input[name=msgFldLen1_"+gbn+"_Source]").val(data["MSGFLDLEN1"]);
						$("input[name=msgFldLen2_"+gbn+"_Source]").val(data["MSGFLDLEN2"]);
						
						$("input[name=serviceId_"+gbn+"_Source]").val(data["SERVICEID"]);
						...
						for( var j = 0; j < transform.length; j++){
							...
							for ( var i = 0; i < trnData.length; i++) {
								...
								var putPosName = $("input[name^=cnvsnName][value="+_CnvsnName+"]").attr("name"); // cnvsnName이 같은 inputbox의 name을 구한다.
...
$(document).ready(function() {	
	...
	$("#btn_new").click(function(){
		...
		setDisable(isDetail);
		$("#btn_modify").attr('src',"<c:url value='/images/bt/bt_regist.gif'/>");
		buttonControl();
		$("#btn_delete").hide();
		$("#tb_clone").hide();
		$("#div_addErrTransform").hide();
		...
		<div class="right_box">
			...
			<div class="content_middle">
				...
				<div style="position:relative;">					
					<table id="tb_clone" class="table_row" cellspacing=0 style="width:50%;"><!-- display:none; -->
						<tr>
					...
					<img src="<c:url value="/img/btn_delete.png"/>" alt="" id="btn_delete" class="btn_img" level="W" status="DETAIL"/>
					<img src="<c:url value="/img/btn_modify.png"/>" alt="" id="btn_modify" class="btn_img" level="W" status="DETAIL,NEW" />	
					<img src="<c:url value="/img/btn_cancel.png"/>" alt="" id="btn_cancel" class="btn_img" level="R" status="DETAIL,NEW" />	> <!-- display:none; -->
					</div>					
				</div>
						
				
				<div id="details">
					<form id="ajaxForm_Common" style="display:block"><!-- display:none --> 					
						<table class="table_row" cellspacing="0">
							...
							<tr>
								...
								<td style="width:30%" ><div class="select-style"><select name="eaiBzwkDstcd" style="width:100%"></select></div></td>
							</tr>
							<tr>
								<th>당타발구분</th>
				...
				<div id="div_addErrTransform" style="margin-bottom:15px;">
				...
				<div class="diagram_wrap">
					...
					<div name="block_REQ" class="table_diagram_wrap" style="display:block"><!-- display:none -->
						<div class="table_diagram">
							...
							<div class="contents_col">	
								<div class="top_wrap">
									<span>서비스ID </span> <input type="text" name="serviceId_REQ_Source" style="width:100px;">
									<br>
									</span><span>참조레이아웃</span>
									<input type="text" name=refIDName_REQ style="width:100px;"> 
									<img src="<c:url value="/img/role_search.png"/>" name="searchRefIDName_REQ" level="W" class="btn_img">
								</div><!-- end.top_wrap -->							
								<div class="middle_wrap" >		
									<div class="arrow_wrap">
										<div class="tale_right"></div>
								...
								<div class="bottom_wrap">
									<div>
										<span>전문추적관리필드</span>
										<input type="text" name="bzwkFldName1_REQ_Source" style="width:100px;">
										<input type="text" name="msgFldStartSituVal1_REQ_Source" size="3" value="0" style="width:30px;">,<input type="text" name="msgFldLen1_REQ_Source" size="3" value="0" style="width:30px;">
										<img src="<c:url value="/img/role_search.png"/>" name="searchbzwkFldName1_REQ_Source" level="W" class="btn_img" />
										<img src="<c:url value="/img/ic_plus.png"/>" name="fldPlus_REQ_Source" level="W" class="btn_img" />
										<div name=fld2_REQ_Source style="display:none;">
											<input type="text" name="bzwkFldName2_REQ_Source" style="width:100px;">
											<input type="text" name="msgFldStartSituVal2_REQ_Source" size="3" value="0" style="width:30px;">,<input type="text" name="msgFldLen2_REQ_Source" size="3" value="0" style="width:30px;">
											<img src="<c:url value="/img/role_search.png" />" name="searchbzwkFldName2_REQ_Source" level="W" class="btn_img" />
											<img src="<c:url value="/img/ic_minus.png"/>" name="fldMin_REQ_Source" level="W" class="btn_img" />
										</div>
									</div>
							...
							<div class="center_col">
								<span>요청변환</span>
								<input type="text" name="cnvsnName_REQ" style="width:112px;" readonly="readonly">
					...
					<div name="block_RES" class="table_diagram_wrap" style="display:block"><!-- display:none -->
						<div class="table_diagram">
							<div class="side_col">
							...
							<div class="center_col">
								<span>응답변환</span>
								<input type="text" name="cnvsnName_RES" style="width:112px;" readonly="readonly">
								<img src="<c:url value="/img/btn_mapping.png"/>" name="mapping_RES" level="R" class="btn_img">
							</div><!-- end.center_col -->
							<div class="contents_col">
								<div class="top_wrap">
									<span>서비스ID</span> <input type="text" name="serviceId_RES_Source" style="width:100px;">
								</div><!-- end.top_wrap -->
								<div class="middle_wrap left" >	
									<div class="arrow_wrap">
										<div class="head_left"></div>
								...
								<div class="bottom_wrap">
									<div>
										<span>참조레이아웃</span> <input type="text" name=refIDName_RES style="width:100px;"> 
										<img src="<c:url value="/img/role_search.png"/>" name="searchRefIDName_RES" level="W" class="btn_img">
									</div>
									<div>
										<span>전문추적관리필드</span> <input type="text" name="bzwkFldName1_RES_Source" style="width:100px;">
										<input type="text" name="msgFldStartSituVal1_RES_Source" size="3" value="0" style="width:30px;">,<input type="text" name="msgFldLen1_RES_Source" size="3" value="0" style="width:30px;"> 
										<img src="<c:url value="/img/role_search.png"/>" name="searchbzwkFldName1_RES_Source" level="W" class="btn_img"> 
										<img src="<c:url value="/img/ic_plus.png"/>" name="fldPlus_RES_Source" level="W" class="btn_img">
										<div name=fld2_RES_Source style="display:none">
											<input type="text" name="bzwkFldName2_RES_Source" style="width:100px;">
											<input type="text" name="msgFldStartSituVal2_RES_Source" size="3" value="0" style="width:30px;">,<input type="text" name="msgFldLen2_RES_Source" size="3" value="0" style="width:30px;"> 
											<img src="<c:url value="/img/role_search.png"/>" name="searchbzwkFldName2_RES_Source" level="W" class="btn_img"> 
											<img src="<c:url value="/img/ic_minus.png"/>" name="fldMin_RES_Source" level="W" class="btn_img">
										</div>
									</div>
								</div><!-- end.bottom_wrap -->	
							</div><!-- end.contents_col -->
					...
					<div name="block_ERR" class="table_diagram_wrap" style="display:block"><!-- display:none -->
						<div class="table_diagram">
							<div class="side_col">
								<img name="adptImg1"  src="<c:url value="/img/psvAdapter1_.png"/>" />
							</div><!-- end.side_col -->
							...
							<div class="center_col">
								<span>에러응답</span>
								<input type="text" name="cnvsnName_ERR" style="width:112px;" readonly="readonly">
							...
							<div class="contents_col">
								...
								<div class="bottom_wrap">
									<div>
										<span>참조레이아웃</span> <input type="text" name="refIDName_ERR" style="width:100px;"> 
				...
				<div name="extAdapter" style="display:block"><!-- display:none -->
