...
function isValidGrid(){
	...
	for ( var i = 0; i < data.length; i++) {
		if (data[i]['DATAKEY'] == $('select[name=addDataKey]').val()
		    && data[i]['DATAVALUE'] == $('input[name=addDataValue]').val()
		    ){
...
function detail(url,key){
	...
	$.ajax({  
		...
		success:function(json){
			...
			$("input[name=useYn]").val(detail['USEYN']);
			//jobItem 
...
$(document).ready(function() {	
	...
	var url ='<c:url value="/onl/admin/scheduler/schedulerMan.json" />';
