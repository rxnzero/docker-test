...
<script language="javascript">
	...
	function isValidGrid() {
		if ($('input[name=prptyId]').val() == "") {
			alert("프러퍼티 키를 입력하여 주십시요.");
			return false;
		} else if ($('input[name=prptyDetail]').val() == "") {
		...
		for ( var i = 0; i < data.length; i++) {
			if (data[i]['PRPTYID'] == $('input[name=prptyId]').val()) {
	...
	function gridRendering() {
		$('#grid').jqGrid({
			...
			colModel : [ { name : 'PRPTYID'     , align : 'left', editable : true },
			             { name : 'PRPTYDETAIL' , align : 'left', editable : true },
	...
	function detail(url, key) {
		...
		$.ajax({
			...
			success : function(json) {
				...
				$("input[name=prptyGroupId]").attr('readonly', true);
				$("input[name=prptyGroupId]").val(detail['PRPTYGROUPID']);
	...
	$(document).ready(function() {
		...
		var url = '<c:url value="/bat/admin/common/propertyMan.json" />';
		var key = "${param.prptyGroupId}";
		...
		$("#btn_pop_input").click(function() {
			...
			var data = new Object();
			data["PRPTYID"]     = $("input[name=prptyId]").val();
			data["PRPTYDETAIL"] = $("input[name=prptyDetail]").val();
			...
			<div class="content_middle">
				...
				<form id="ajaxForm">
					<table class="table_row" cellspacing="0">
						<tr>
							...
							<td><input type="text" name="prptyGroupId"></td>
						</tr>
						<tr>
							<th>프라퍼티 그룹설명 *</th>
				...
				<table class="table_row" cellspacing="0">
					<tr>
						...
						<td><input type="text" name="prptyId" style="width:calc(100% - 85px); margin-right:10px;"><img src="<c:url value="/img/btn_pop_input.png" />" id="btn_pop_input" class="btn_img" /></td>						
					</tr>
					<tr>
						<th>프라퍼티 값</th>
						<td><input type="text" name="prptyDetail"></td>
					</tr>
				</table>
				<!-- grid -->
				<table id="grid"></table>
