...
function titleControl(key){
	...
	if (isStatus){
		obj.html("Edit"+" "+obj.html());
	}else{
		obj.html("New Registration"+" "+obj.html());
	}	
...
function gridToExcelSubmit(url,cmd,gridObj,formObject,fileName,merge){
	...
	for ( var i = 0; i < data.length; i++) {
		for (key in data[i]) {
			data[i][key]=encodeURI(data[i][key]);
		}
	...
	for(var i=0;i<colM.length;i++){
		if (colM[i].hidden){
			...
			headers.push(encodeURI(colM[i].name));
			headerTitles.push(encodeURI(colN[i]));
		}
	}
	postData.push({
		name : "header",
