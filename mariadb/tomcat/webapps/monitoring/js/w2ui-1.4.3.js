...
/* customizing
 * add MergeRow()
 * Array.indexOf is not available below version IE9 , so it changes to $.inArray(jQuery)
 *  */
/************************************************
...
(function () {
    var w2grid = function(options) {
        ...
        this.mergeRows = [];			//merge cols ,ex) mergeRows=[0,1]
        ...
        this.keyboard       = false;	//change true to false , lim 20161115
        this.selectType     = 'row';    // can be row|cell
        this.multiSearch    = false;	//change true to false , lim 20161115
        this.multiSelect    = false;	//change true to false , lim 20161115
        this.multiSort      = false;	//change true to false , lim 20161115
        this.reorderColumns = false;
        this.reorderRows    = false;
        this.markSearch     = false;	//change true to false , lim 20161115
        this.mergeInit		= false;	//change true to false , lim 20161115
    ...
    w2grid.prototype = {
        ...
        select: function () {
            ...
            for (var a = 0; a < arguments.length; a++) {
                ...
                if (this.selectType == 'row') {
                    if ($.inArray(index,sel.indexes) >= 0) continue;
                    // event before
                    ...
                    if (eventData.isCancelled === true) continue;
                    // default action
                    if ($.inArray(index,sel.indexes) == -1) {
                        sel.indexes.push(index);
                        sel.indexes.sort(function(a, b) { return a-b });
                    }
                    s.push(col);
        ...
        click: function (recid, event) {
            ...
            if (time - parseInt(this.last.click_time) < 1100 && this.last.click_recid == recid && event.type == 'click') {
            ...
            if (event.shiftKey && sel.length > 0 && obj.multiSelect) {
                ...
                var flag = ($.inArray(ind,last.indexes) != -1 ? true : false);
        ...
        contextMenu: function (recid, event) {
            ...
            if ($.inArray(recid,this.getSelection()) == -1) obj.click(recid);
        ...
        render: function (box) {
            ...
            function mouseMove (event) {
                ...
                if (obj.selectType != 'row') {
                    ...
                    if (obj.multiSelect) {
                        var sel = obj.getSelection();
                        for (var ns in newSel) if ($.inArray(ns,sel) == -1) obj.select(newSel[ns]); // add more items
                        for (var s in sel) if ($.inArray(s,newSel) == -1) obj.unselect(sel[s]); // remove items
                    }
                }
            }
        ...
        scroll: function (event) {
            ...
            if (!url && (!this.fixedBody || this.total <= 300)){
            	if(!this.mergeInit){
            		mergeRow(this, this.mergeRows);
            		this.mergeInit = true;
            	}
            	return;
            }
            ...
            if (first < start || first == 1 || this.last.pull_refresh) { // scroll down
            	...
            	if (end <= last + this.show_extra - 2 && end != this.total){
                	return;
                }
                ...
                if (start >= first - this.show_extra + 2 && start > 1){
                	if(!this.mergeInit){
                		mergeRow(this, this.mergeRows);
                		this.mergeInit = true;
                	}                	
            ...
            function mergeRow(obj, colNums){
            	var records = $('#grid_'+ obj.name +'_records');
                var top = records.find('#grid_'+ obj.name +'_rec_top');
                var bottom = records.find('#grid_'+ obj.name +'_rec_bottom');            
                var mergeStart = parseInt(top.next().attr('line'));
                var mergeEnd = parseInt(bottom.prev().attr('line'))
                
                
                var rowCnt = 1;
                         
               
                for (var i = mergeStart; i <= mergeEnd; i++) { //init table
                	$('#grid_'+ obj.name +'_rec_'+i).find('td').each(function(){ $(this).show();});
                	$('#grid_'+ obj.name +'_rec_'+i).find('td').each(function(){ $(this).attr('rowspan',1);});
                }
                ...
                for(var col in colNums){
                    ...
                    var cnt = 1;
                    var startRow = mergeStart;
                    var firstRowVal ="";
                    
                	for (var i = mergeStart; i <= mergeEnd; i++) {             	
	                	if(i == mergeStart ){
	                		firstRowVal = obj.getCellValue(i-1, colNum, false);
	                		continue;
	                	}
	             	   var cellVal = obj.getCellValue(i-1, colNum, false);
	             	  
	             	   if(firstRowVal== cellVal){
	             		  if(i ==mergeEnd ){
	              			 $('#grid_'+ obj.name +'_rec_'+i).find('td:eq('+colNum+')').hide();
	             			 $('#grid_'+ obj.name +'_rec_'+startRow).find('td:eq('+colNum+')').attr('rowspan',cnt+1);
	             		  }else{
	                 		   $('#grid_'+ obj.name +'_rec_'+i).find('td:eq('+colNum+')').hide();
	                 		  cnt++;
	             		   continue;
	             		   ...
	             		   $('#grid_'+ obj.name +'_rec_'+startRow).find('td:eq('+colNum+')').attr('rowspan',cnt);
        ...
        getRecordHTML: function (ind, lineNum, summary) {
            ...
            if ($.inArray(ind,sel.indexes) != -1) isRowSelected = true;
            // render TR
